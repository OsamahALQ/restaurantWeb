...
function getMenuHTML(rest){
	...
	Object.keys(menu).forEach(key =>{
		...
		Object.keys(menu[key]).forEach(id => {
			item = menu[key][id];
			result += `${item.name} (\$${item.price}) <img src="views/images/add.png" style='height:20px;vertical-align:bottom;' onclick='addItem(${id})'/> <br>`;
...
function updateOrder(){
	...
	Object.keys(order).forEach(id =>{
		...
		result += `${item.name} x ${order[id].quantity} (${(item.price * order[id].quantity).toFixed(2)}) <img src='views/images/remove.png' style='height:15px;vertical-align:bottom;' onclick='removeItem(${id})'/><br>`;
	});
	...
	if(currentSubtotal >= currentRestaurant.min_order){
		result += `<button type="button" id="submit" onclick="submitOrder()" style="background: linear-gradient(to left, #4776E6, #8e54e9); color: white; border-radius: 8px; margin-top: .8rem;">Submit Order</button>`
	}else{
